...
export interface WorkflowPageOptions { businessProfileId?: string; nextToken?: string; limit?: number }
interface Filters { businessProfileId?: string; workflowId?: string; runId?: string; nodeId?: string; status?: string; isActive?: boolean; search?: string }
...
export async function workflowPage<T>(db: IDdb, orgId: string, prefix: string, options: WorkflowPageOptions, filters: Filters = {}): Promise<{ items: T[]; nextToken?: string }> {
    ...
    for (const field of ['businessProfileId', 'workflowId', 'runId', 'nodeId', 'status', 'isActive'] as const) {
