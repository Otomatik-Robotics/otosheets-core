...
const row = (orgId: string, w: Record<string, any>) => ({ orgId: workflowScope(orgId), workflowId: w.workflowId, name: w.name, isActive: w.isActive, updatedAt: w.updatedAt, payload: clean({ ...w, orgId, sk: `WORKFLOW#${w.workflowId}` }) });
export class OnboardingWorkflowPgRepo implements IOnboardingWorkflowRepo {
    ...
    async saveVersion(orgId: string, w: Omit<OnboardingWorkflow, 'orgId' | 'sk'>, expectedVersion: number, saveKey: string): Promise<{ status: 'saved' | 'replayed' | 'conflict'; version: number }> {
        ...
        const fingerprint = createHash('sha256').update(JSON.stringify({ name: w.name, description: w.description, isActive: w.isActive, nodes: w.nodes, edges: w.edges })).digest('hex');
        return this.db.transaction(async tx => {
            ...
            const payload = clean({ orgId, workflowId: w.workflowId, version, nodes: w.nodes, edges: w.edges, createdAt: w.updatedAt, createdBy: w.updatedBy ?? w.createdBy ?? '', saveKey, fingerprint, sk: `VERSION#${w.workflowId}#${String(version).padStart(6, '0')}` });
            await tx.insert(versions).values({ orgId, workflowId: w.workflowId, version, payload });
    ...
    async listPage(orgId: string, options: WorkflowPageOptions & { search?: string; isActive?: boolean } = {}) {
        workflowScope(orgId); const scope = [orgId, 'definitions', options.search ?? null, options.isActive ?? null]; const key = pageToken(options.nextToken, scope); const limit = pageLimit(options.limit);
        const rows = await this.db.select().from(defs).where(and(eq(defs.orgId, orgId), options.search ? ilike(defs.name, `%${options.search.replace(/[\\%_]/g, '\\$&')}%`) : undefined, options.isActive === undefined ? undefined : eq(defs.isActive, options.isActive), key ? or(lt(defs.updatedAt, key[0]), and(eq(defs.updatedAt, key[0]), lt(defs.workflowId, key[1]))) : undefined)).orderBy(desc(defs.updatedAt), desc(defs.workflowId)).limit(limit + 1);
    ...
    async listVersionsPage(orgId: string, workflowId: string, options: WorkflowPageOptions = {}) {
        const scope = [orgId, 'versions', workflowId]; const key = pageToken(options.nextToken, scope); const limit = pageLimit(options.limit);
        const rows = await this.db.select().from(versions).where(and(versionWhere(orgId, workflowId), key ? lt(versions.version, key[0]) : undefined)).orderBy(desc(versions.version)).limit(limit + 1);
    ...
    async list(orgId: string): Promise<OnboardingWorkflow[]> { return (await this.db.select().from(defs).where(eq(defs.orgId, workflowScope(orgId)))).map(r => r.payload as OnboardingWorkflow); }
    async countByOrg(orgId: string): Promise<number> { return Number((await this.db.select({ value: count() }).from(defs).where(eq(defs.orgId, workflowScope(orgId))))[0].value); }
