...
describe('invoice overdue pagination', () => {
    it('filters by client, strict cutoff and status before paging', async () => {
        ...
        const rows = [
            ['sc-a', 'scope-client', 'SENT', '2026-01-01'],
            ['sc-partial', 'scope-client', 'PARTIAL', '2026-01-01'],
            ['sc-client', 'other-scope-client', 'SENT', '2026-01-01'],
            ['sc-today', 'scope-client', 'SENT', '2026-02-01'],
            ['sc-paid', 'scope-client', 'PAID', '2026-01-01'],
            ['sc-draft', 'scope-client', 'DRAFT', '2026-01-01'],
        ];
        for (const [id, clientId, status, dueDate] of rows) {
            await repo.createInvoice('org_1', 'u', id, { invoiceNumber: id, clientId, status, dueDate });
        }
        const filter = { orgId: 'org_1', clientId: 'scope-client', overdueBefore: '2026-02-01', limit: 1 };
