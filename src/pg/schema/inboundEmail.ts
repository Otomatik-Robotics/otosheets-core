...
const scope = () => ({ orgId: text('org_id').notNull(), businessProfileId: text('business_profile_id').notNull() });
...
}, t => [primaryKey({ columns: [t.orgId, t.businessProfileId] }), uniqueIndex('inbound_mailboxes_address_uq').on(t.address)]);
...
}, t => [primaryKey({ columns: [t.orgId, t.businessProfileId, t.conversationId] }), uniqueIndex('email_conversations_reply_uq').on(t.replyAddress), index('email_conversations_invoice_idx').on(t.orgId, t.businessProfileId, t.invoiceId)]);
...
}, t => [primaryKey({ columns: [t.orgId, t.businessProfileId, t.messageId] }), index('inbound_messages_profile_received_idx').on(t.orgId, t.businessProfileId, t.receivedAt, t.messageId)]);
