...
import { BookingDynamoRepo, type IBookingRepo, type BookingListParams } from './repo';
import { BookingPgRepo } from './repo.pg';

const DOMAIN = 'leads' as const;
const ENTITY = 'booking';
type PagParams = BookingListParams;
...
export class RoutingBookingRepo implements IBookingRepo {
    ...
    async listBookingsByDate(o: string, f: string, t: string, businessProfileId?: string) { const r = await resolveRoute(DOMAIN); return this.read('listBookingsByDate', () => this.pick(r).listBookingsByDate(o, f, t, businessProfileId), () => this.pg.listBookingsByDate(o, f, t, businessProfileId), r); }
    async listBookingsByLead(o: string, leadId: string) { const r = await resolveRoute(DOMAIN); return this.read('listBookingsByLead', () => this.pick(r).listBookingsByLead(o, leadId), () => this.pg.listBookingsByLead(o, leadId), r); }
